...
def clostestSegment(img, labelX, labelY, i, j):
    y1 = np.where(labelX[i] == labelX[i, j])[0][0]
    x1 = np.where(labelY[:, j] == labelY[i, j])[0][0]
    y2 = np.where(labelX[i] == labelX[i, j])[0][-1]
    x2 = np.where(labelY[:, j] == labelY[i, j])[0][-1]
    if min(i-x1, x2-i) < min(j-y1, y2-j):
        return img[x1-1, j], False
    else:
        if min(i-x1, x2-i) == min(j-y1, y2-j):
            return img[i, y1-1], True
        return img[i, y1 - 1], False
    ...
    for i in range(len(img)):
        for j in range(len(img[i])):
            if img[i,j] == 0:
                ...
                a,b,c,d = 0,0,0,0
                img_out[i,j], equal = clostestSegment(img, labelX, labelY, i,j)
                if equal:
                    c, d = 0.5, 0.5
            else:
